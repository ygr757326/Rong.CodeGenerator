...
namespace Rong.Volo.Abp.CodeGenerator.Vue.TemplateHelpers.Vben2
{
    ...
    /// vben模板 -  表头 columns
    /// </summary>
    public class RongVoloAbpVueVben2TemplateStringOfTableColumns : RongVoloAbpVueTemplateBase, ISingletonDependency
    {

        public RongVoloAbpVueVben2TemplateStringOfTableColumns(IOptions<RongVoloAbpCodeGeneratorVueOptions> options) : base(options)
        {
        }
        /// <summary>
        /// 忽略默认模板属性
        /// <para>若逻辑不符，请重写该方法</para>
        /// </summary>
        /// <returns></returns>
        ...
        public virtual string? FilePreviewSlot(TemplateVueEntityPropertyData item, int space = 6)
        {
            ...
            {
                b.Append(
                    $" :{Options.FilePreviewComponentProp ?? "v-model"}=\"value\"  listType=\"text\" :disabled=\"true\" ");
            }
            else
            {
                b.Append(
