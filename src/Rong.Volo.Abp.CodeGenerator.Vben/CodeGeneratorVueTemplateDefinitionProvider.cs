    ...
    public class CodeGeneratorVueTemplateDefinitionProvider : TemplateDefinitionProvider
    {
        public override void Define(ITemplateDefinitionContext context)
        {
            ...
            foreach (var item in templates)
            {
                ...
                var notInComponents = item == CodeGeneratorVbenTemplateNames.Vben_index || item == CodeGeneratorVbenTemplateNames.Vben_api;
                def.WithProperty("path", $"$rootPath/src/views/xxx{(notInComponents ? null : "/components")}");


                if (item == CodeGeneratorVbenTemplateNames.Vben_api)
                {
                    def.WithProperty("name", $"xxx{name}.ts");
                }
